...
同步表明程序或者过程的执行是顺序的，每个步骤的执行都需要上一个步骤完成，才能继续
异步表明程序或者过程的执行时顺序的，但是每个步骤的执行不需要上一个步骤完成，而是给每个步骤提供完成后的通知，让主程序进行注册然后执行
阻塞主要用来表示程序的某些状态不符合该操作继续进行的条件，而需要等待该状态转移才能继续进行
非阻塞主要通过将程序的操作与程序的状态分开，而不是执行操作前等待程序的状态符合条件，是通过检查程序状态是否符合条件，然后选择后续的操作，不存在程序阻塞在状态上

## 22. 什么是线程池（thread pool）？线程池的原理是什么？//TODO
...
## 49. 自旋锁，轻量级锁，偏向锁，重量级锁的区别是什么？

自旋锁：
互斥同步对性能最大的影响是阻塞的实现，挂起线程和恢复线程都需要转入内核态，如果处理器只有为数不多的线程，
则不需要挂起线程，只需要让该线程继续处理器的执行时间，进行自旋等待。
jdk6后，自适应的自旋锁，意味着自选的时间不再固定。
轻量级锁：
jdk6后，减少传统的重量级锁使用操作系统互斥量产生的性能消耗。
如果有两条以上的线程征用同一个锁，那轻量级锁就不在有效，膨胀为重量级锁。
偏向锁：
jdk6后，消除数据在无竞争情况下的同步原语，进一步提供程序的运行性能。
和轻量级锁对比来说，轻量级锁通过CAS进行mark word的状态的更新，而偏向锁直接将整个同步消除掉。
当有另外一个线程去尝试获取这个锁时，偏向模式就接护送了，根据锁对象目前处于的被锁定状态进行撤销。
